...
public class FightButton extends Button
{
    ...
    public void select() {
        IPokemon player = world.player; // get current player object
    ...
    public void greyOutIfDead() {
        if(world.player.getWorld() != null) { // if player exists
            ...
            ICommand[][] grid = getWorld().getObjects(Selection.class).get(0).getGrid();
            ICommand[][] tempGrid = new ICommand[grid.length][grid[0].length - 1];
